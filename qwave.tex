...
\notag
	...
	\\[4mm]
%
	\psi_L^\dag		& ~~\to~~		e^{-i\varphi}\,\psi_L^\dag\,,
	&
	\psi_R^\dag		& ~~\to~~		e^{-i\varphi}\,\psi_R^\dag\,.
\end{align}
...
\end{equation}
	We did not make anything up here, as the first term
	was just hidden inside the ``$ \text{q.c.} $'' and ``$ \text{c.c.} $'' in Eq.~\eqref{L-massive}.
	The noticeable difference is that this term enters with a \emph{positive} sign.
