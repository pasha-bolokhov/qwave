...
\]
	In the first equation in \eqref{dirac-standard}, the mass term is now gone:
\begin{equation}
	i\,\p_0\,\psi_+		~~=~~	(\vec\nabla ~+~ i\,\vec A)\,\psi_-  ~~-~~  A_0\, \psi_+\,,
\end{equation}
	while in the second term the mass term doubles.
...
\]
	because the $ \sigma $-matrices are hermitean.
	Second (and actually the same), as magnetic field $ \vec B $ is an axial vector,
	multiplying it by $ i $ turns it into a polar vector. 
