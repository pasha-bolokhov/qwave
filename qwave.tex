...
\setcounter{equation}{0}
	...
	The list of literature on using quaternions in physics is so vast that it is hardly
	possible to be overseen \cite{}.
	The majority of the literature touches base on the use of quaterions for three-dimensional
	rotation and Lorentz transformations.
	Other papers include their applications in Electrodynamics and Quantum Mechanics.
	The attractive feature of quaternions is that whenever a solution is possible in quaternionic form,
	it is much more compact than that in customary form of Lorentz vectors and tensors. 
	...
	The other drawback, as it is perceived, is a somewhat ``strange'' language, and often strange results
	following from this language.
	While from learning Quantum Mechanics and Gauge Theories we are used to non-commutative operators,
	and non-commutative objects in general, when we see an operator of multiplication ``from the right'',
	it immediately induces a certain degree of skepsis.
	That is, mathematically this formalism may be interesting, but physically this seems to be
	driven away from reality.
	Furthermore, some treatments on quaternions in Quantum Mechanics predict some extra degrees of freedom for fermions,
	which we obviously do not observe.
	This is the result of the fact that quaternions are multi-component numbers, a bit too many for Quantum Mechanics.
	...
	That is, to say, that the wavefunction is complex.
	...
	The algebra of Pauli matrices is the same as that of quaternionic units.
	...
	and the resulting Dirac or Schr\"odinger equation then includes non-physical degrees of freedom.
	We proceed in a very conservative way:
	...
	Quaternionic derivations, while a bit unusual for some, are simpler.
...
\vskip 0.6cm
	...
	We overview the known facts which are easy to pick up and use, while their proof can be found in the literature.
...
\end{align}
	...
	We denote the quaternionic conjugtion by $ \wt a $,
\begin{equation}
...
\end{equation}
	...
	\emph{So let's talk about them}

	Another way a complex quaternion \eqref{cq} can be split up, is by separating its zeroth component $ a^0 $
	and its vector part $ \vec a $.
	The zeroth component $ a^0 $ is identified as a complex scalar field.
...
\end{equation}
	...
	While both $ \vec B $ and $ \vec E $ are vectors in the three-dimensional sence,
	one does not view the object \eqref{fs} as a four-dimensional vector in any sence.
...
\item Vectors and pseudo-vectors

\item Scalars and field strength

\item Spinors

\item Standard Model as the ultimate goal
